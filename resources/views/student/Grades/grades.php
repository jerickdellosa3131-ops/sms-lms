...
if ($student) {
    ...
    $grades = DB::table('grades')
        ...
        ->select('grades.*', 'classes.section_name', 'classes.class_code', 'grade_components.component_name')
        ->orderBy('grades.graded_at', 'desc')
        ->get();
    
    // Calculate overall average
    $overallAvg = $grades->where('percentage', '>', 0)->avg('percentage');
} else {
    $grades = collect();
    $overallAvg = 0;
}
  ...
  <div class="main-content flex-grow-1">
    ...
    <div class="container-fluid">
      ...
      <div class="page-header mb-4">
          <h2 class="display-6"><i class="bi bi-calendar-week me-3"></i>My Grades</h2>
          <p class="text-muted">Overall Average: <strong><?php echo round($overallAvg, 2); ?>%</strong></p>
      </div>

      <?php if($grades->count() > 0): ?>
        <?php 
        // Group grades by class
        $gradesByClass = $grades->groupBy('class_id');
        foreach($gradesByClass as $classId => $classGrades): 
            $firstGrade = $classGrades->first();
        ?>
        <div class="card mb-4">
            <div class="card-header theme-bg fw-bold">
                <i class="bi bi-check2-circle me-2"></i> <?php echo htmlspecialchars($firstGrade->section_name); ?>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="text-center">
                            <tr>
                                <th class="text-start ps-3">Component</th>
                                <th>Score</th>
                                <th>Max Score</th>
                                <th>Percentage</th>
                                <th>Remarks</th>
                        ...
                        <tbody class="text-center">
                            ...
                            <tr>
                                <td class="text-start ps-3 fw-bold"><?php echo htmlspecialchars($grade->component_name ?? 'Assignment'); ?></td>
                                <td class="score <?php echo $grade->percentage >= 75 ? 'text-success' : 'text-warning'; ?>"><?php echo $grade->score; ?></td>
                                <td class="score"><?php echo $grade->max_score; ?></td>
                                <td class="score <?php echo $grade->percentage >= 75 ? 'text-success' : 'text-warning'; ?>"><?php echo round($grade->percentage, 1); ?>%</td>
                                <td><span class="badge rounded-pill status-graded px-3"><?php echo htmlspecialchars($grade->remarks ?? 'Graded'); ?></span></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
      ...
      <?php else: ?>
        <div class="alert alert-info">
            <i class="bi bi-info-circle me-2"></i>No grades available yet.
        </div>
      <?php endif; ?>
